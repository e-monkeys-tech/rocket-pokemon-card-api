...
fn test_crud_pokemon() {
    ...
    let mut response = client
        ...
        .body(
            serde_json::to_string(&PokemonCreate {
                id: None,
                username: "pokemon1".to_owned(),
                is_active: false,
            })
            .unwrap(),
        )
    ...
    let created_body: Pokemon =
        serde_json::from_str(response.body_string().unwrap().as_str()).unwrap();

    // Read pokemon
    let mut response = client
        .get(format!("/api/pokemon_cards/{}", created_body.id))
        .header(ContentType::JSON)
        .dispatch();

    let fetched_body: Pokemon =
        serde_json::from_str(response.body_string().unwrap().as_str()).unwrap();
    ...
    // Update user
    let mut response = client
        .patch("/api/pokemon_cards")
        .body(
            serde_json::to_string(&PokemonUpdate {
                id: fetched_body.id,
                username: Some("pokemon1_modified".to_owned()),
                is_active: Some(true),
            })
            .unwrap(),
        )
    ...
    let updated_body: Pokemon =
        serde_json::from_str(response.body_string().unwrap().as_str()).unwrap();

    assert_eq!(response.status(), Status::Ok);
    assert_eq!(updated_body.id, fetched_body.id);
    assert_eq!(updated_body.username, "pokemon1_modified".to_string());
    assert_eq!(updated_body.is_active, true);

    // Delete pokemon
    let mut response = client
        .delete(format!("/api/pokemon_cards/{}", updated_body.id))
        .header(ContentType::JSON)
        .dispatch();

    let deleted_body: Pokemon =
        serde_json::from_str(response.body_string().unwrap().as_str()).unwrap();
    assert_eq!(response.status(), Status::Ok);
    assert_eq!(deleted_body.id, updated_body.id);
    assert_eq!(deleted_body.username, updated_body.username);
